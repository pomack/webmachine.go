...
func (p *wmDecisionCore) decision(decisionId WMDecision) WMDecision {
  ...
  switch decisionId {
  case v3b13: nextDecision = p.doV3b13() // Service available?
  case v3b13b: nextDecision = p.doV3b13b()
  case v3b12: nextDecision = p.doV3b12() // Known method?
  case v3b11: nextDecision = p.doV3b11() // URI too long?
  case v3b10: nextDecision = p.doV3b10() // Method allowed?
  case v3b9: nextDecision = p.doV3b9() // Malformed?
  case v3b8: nextDecision = p.doV3b8() // Authorized?
  case v3b7: nextDecision = p.doV3b7() // Forbidden?
  case v3b6: nextDecision = p.doV3b6() // Okay Content-* Headers?
  case v3b5: nextDecision = p.doV3b5() // Known Content-Type?
  case v3b4: nextDecision = p.doV3b4() // Req Entity Too Large?
  case v3b3: nextDecision = p.doV3b3() // OPTIONS?
  case v3c3: nextDecision = p.doV3c3() // Accept exists?
  case v3c4: nextDecision = p.doV3c4() // Acceptable media type available?
  case v3d4: nextDecision = p.doV3d4() // Accept-Language exists?
  case v3d5: nextDecision = p.doV3d5() // Acceptable Language available?
  case v3e5: nextDecision = p.doV3e5() // Accept-Charset exists?
  case v3e6: nextDecision = p.doV3e6() // Accceptable Charset available?
  case v3f6: nextDecision = p.doV3f6() // Accept-Encoding exists?
  case v3f7: nextDecision = p.doV3f7() // Acceptable encoding available?
  case v3g7: nextDecision = p.doV3g7() // Resource exists?
  case v3g8: nextDecision = p.doV3g8() // If-Match exists?
  case v3g9: nextDecision = p.doV3g9() // If-Match: * exists
  case v3g11: nextDecision = p.doV3g11() // ETag in If-Match
  case v3h7: nextDecision = p.doV3h7() // If-Match: * exists
  case v3h10: nextDecision = p.doV3h10() // If-unmodified-since exists?
  case v3h11: nextDecision = p.doV3h11() // I-UM-S is valid date?
  case v3h12: nextDecision = p.doV3h12() // Last-Modified > I-UM-S?
  case v3i4: nextDecision = p.doV3i4() // Moved permanently?
  case v3i7: nextDecision = p.doV3i7() // PUT?
  case v3i12: nextDecision = p.doV3i12() // If-none-match exists?
  case v3i13: nextDecision = p.doV3i13() // If-None-Match: * exists?
  case v3j18: nextDecision = p.doV3j18() // GET or HEAD?
  case v3k5: nextDecision = p.doV3k5() // Moved permanently?
  case v3k7: nextDecision = p.doV3k7() // Previously existed?
  case v3k13: nextDecision = p.doV3k13() // Etag in if-none-match?
  case v3l5: nextDecision = p.doV3l5() // Moved temporarily?
  case v3l7: nextDecision = p.doV3l7() // POST?
  case v3l13: nextDecision = p.doV3l13() // IMS exists?
  case v3l14: nextDecision = p.doV3l14() // IMS is valid date?
  case v3l15: nextDecision = p.doV3l15() // IMS > Now?
  case v3l17: nextDecision = p.doV3l17() // Last-Modified > IMS?
  case v3m5: nextDecision = p.doV3m5() // POST?
  case v3m7: nextDecision = p.doV3m7() // Server allows POST to missing resource?
  case v3m16: nextDecision = p.doV3m16() // DELETE?
  case v3m20: nextDecision = p.doV3m20() // DELETE enacted immediately?
  case v3m20b: nextDecision = p.doV3m20b() //
  case v3n5: nextDecision = p.doV3n5() // Server allows POST to missing resource?
  case v3n11: nextDecision = p.doV3n11() // Redirect?
  case v3n16: nextDecision = p.doV3n16() // POST?
